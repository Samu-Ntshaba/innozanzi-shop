...
function Stars({ rating, size = "size-4" }: { rating: number; size?: string }) {
  return <div className="flex text-amber-500">{[1, 2, 3, 4, 5].map(star => <Star className={`${size} ${star <= rating ? "fill-current" : "text-slate-300"}`} key={star}/>)}</div>;
}
  ...
  return <section id="reviews" className="mt-14 scroll-mt-28 border-t border-slate-200 pt-10">
    <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,.72fr)_minmax(0,1.28fr)]">
      <div>
        ...
        {reviews.length ? <div className="mt-4 flex items-center gap-3"><strong className="text-4xl text-slate-950">{average.toFixed(1)}</strong><div><Stars rating={Math.round(average)} size="size-5"/><p className="mt-1 text-xs text-slate-500">Based on {reviews.length} approved {reviews.length === 1 ? "review" : "reviews"}</p></div></div> : <p className="mt-4 text-sm leading-6 text-slate-600">No approved reviews yet. Be the first customer to share useful feedback.</p>}
      ...
      <div className="space-y-4">
        {reviews.map(review => <article className="rounded-xl border border-slate-200 bg-white p-5" key={review.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><Stars rating={review.rating}/><h3 className="mt-2 font-bold text-slate-950">{review.title ?? "Customer review"}</h3></div><time className="text-xs text-slate-500">{review.createdAt.toLocaleDateString("en-ZA")}</time></div>{review.body ? <p className="mt-3 whitespace-pre-line text-sm leading-6 text-slate-700">{review.body}</p> : null}<div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-slate-500"><span className="inline-flex items-center gap-1"><MessageSquare className="size-3.5"/>{review.user.name ?? "Innozanzi customer"}</span>{review.isVerifiedPurchase ? <span className="inline-flex items-center gap-1 font-semibold text-emerald-700"><CheckCircle2 className="size-3.5"/>Verified purchase</span> : null}</div></article>)}
