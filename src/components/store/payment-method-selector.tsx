...
function MethodOption({ method, selected, setMethod }: { method: Method; selected: boolean; setMethod: (method: Method) => void }) {
  const Icon = method === "OZOW" ? Landmark : CreditCard;
  const title = method === "OZOW" ? "Pay with Ozow" : "Pay with PayFast";
  const detail = method === "OZOW" ? "Secure bank payment" : "Secure online card payment";
...
export function PaymentMethodSelector({ total, available = { OZOW: false, PAYFAST: false } }: { total: string; available?: Record<Method, boolean> }) {
  const [method, setMethod] = useState<Method>(available.OZOW ? "OZOW" : "PAYFAST");
  const noMethods = !available.OZOW && !available.PAYFAST;
  return <div className="p-5 sm:p-6">
    ...
    {noMethods ? <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-900">Online payment is temporarily unavailable. Please contact support.</p> : method === "PAYFAST" ? <p className="mt-4 rounded-lg bg-emerald-50 p-3 text-xs leading-5 text-emerald-800">You’ll continue to PayFast’s secure payment page.</p> : <p className="mt-4 rounded-lg bg-amber-50 p-3 text-xs leading-5 text-amber-900">You’ll continue to Ozow to authorise payment securely with your bank.</p>}
