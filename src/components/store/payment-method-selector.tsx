...
import { Building2, Check, CreditCard, Landmark, LoaderCircle, LockKeyhole } from "lucide-react";

type Method = "PAYFAST" | "OZOW" | "EFT";

function SubmitButton({ method, total, available }: { method: Method; total: string; available: boolean }) {
  const { pending } = useFormStatus();
  return <button disabled={pending || !available} className="mt-5 flex min-h-13 w-full items-center justify-center gap-2 rounded-lg bg-sky-700 px-5 text-base font-bold text-white transition hover:bg-sky-800 disabled:bg-slate-400">{pending ? <><LoaderCircle className="size-5 animate-spin"/>Processing…</> : method === "EFT" ? <>Continue with EFT</> : <>Pay {total} securely with {method === "PAYFAST" ? "PayFast" : "Ozow"}</>}</button>;
}

function MethodOption({ method, selected, setMethod }: { method: Method; selected: boolean; setMethod: (method: Method) => void }) {
  const Icon = method === "PAYFAST" ? CreditCard : method === "OZOW" ? Landmark : Building2;
  const title = method === "PAYFAST" ? "Pay with PayFast" : method === "OZOW" ? "Pay with Ozow" : "Pay by EFT";
  const detail = method === "PAYFAST" ? "Secure card payment and more" : method === "OZOW" ? "Secure instant bank payment" : "Pay immediately, then upload proof of payment";
  const cardClass = "relative flex cursor-pointer gap-3 rounded-xl border-2 p-4 transition focus-within:ring-2 focus-within:ring-sky-700 focus-within:ring-offset-2 " + (selected ? "border-sky-700 bg-sky-50 shadow-sm" : "border-slate-200 bg-white hover:border-slate-300");
...
export function PaymentMethodSelector({ total, available = { PAYFAST: false, OZOW: false, EFT: false } }: { total: string; available?: Record<Method, boolean> }) {
  const [method, setMethod] = useState<Method>(available.PAYFAST ? "PAYFAST" : available.OZOW ? "OZOW" : "EFT");
  const noMethods = !available.PAYFAST && !available.OZOW && !available.EFT;
  return <div className="p-5 sm:p-6">
    ...
    <fieldset className="mt-4 space-y-3"><legend className="sr-only">Choose payment method</legend>
      {available.PAYFAST ? <MethodOption method="PAYFAST" selected={method === "PAYFAST"} setMethod={setMethod}/> : null}
    ...
    {noMethods ? <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-900">Online payment is temporarily unavailable. Please contact support.</p> : method === "EFT" ? <p className="mt-4 rounded-lg bg-sky-50 p-3 text-xs leading-5 text-sky-900"><strong>Make your EFT payment immediately after placing the order.</strong> We will only process and fulfil your order once the payment has reflected in our bank account and has been verified.</p> : <p className="mt-4 rounded-lg bg-amber-50 p-3 text-xs leading-5 text-amber-900">{method === "PAYFAST" ? "You’ll continue to PayFast to enter your card details or choose another available payment method. Your card details stay with PayFast." : "You’ll continue to Ozow to authorise payment securely with your bank."}</p>}
