...
function SubmitButton({ method, total, available }: { method: Method; total: string; available: boolean }) {
  ...
  return <button disabled={pending || !available} className="mt-5 flex min-h-13 w-full items-center justify-center gap-2 rounded-lg bg-sky-700 px-5 text-base font-bold text-white transition hover:bg-sky-800 disabled:bg-slate-400">{pending ? <><LoaderCircle className="size-5 animate-spin"/>Processing…</> : <>Pay {total} securely with {method === "PAYFAST" ? "PayFast" : "Ozow"}</>}</button>;
}

function MethodOption({ method, selected, setMethod }: { method: Method; selected: boolean; setMethod: (method: Method) => void }) {
  const Icon = method === "PAYFAST" ? CreditCard : Landmark;
  const title = method === "PAYFAST" ? "Pay with PayFast" : "Pay with Ozow";
  const detail = method === "PAYFAST" ? "Secure card payment and more" : "Secure instant bank payment";
...
export function PaymentMethodSelector({ total, available = { PAYFAST: false, OZOW: false } }: { total: string; available?: Record<Method, boolean> }) {
  const [method, setMethod] = useState<Method>(available.PAYFAST ? "PAYFAST" : "OZOW");
  const noMethods = !available.PAYFAST && !available.OZOW;
  return <div className="p-5 sm:p-6">
    ...
    {noMethods ? <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-900">Online payment is temporarily unavailable. Please contact support.</p> : <p className="mt-4 rounded-lg bg-amber-50 p-3 text-xs leading-5 text-amber-900">{method === "PAYFAST" ? "You’ll continue to PayFast to enter your card details or choose another available payment method. Your card details stay with PayFast." : "You’ll continue to Ozow to authorise payment securely with your bank."}</p>}
