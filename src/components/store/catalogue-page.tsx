...
export async function CataloguePage({ params, heading = "Shop technology" }: { params: CatalogueParams; heading?: string }) {
  ...
  const [result, adminTestProducts] = await Promise.all([
    ...
    canAccessTestProducts(context) ? getAdminTestProducts() : Promise.resolve([]),
  ]);
  ...
  const filters = <>
    ...
    <select className="h-12 min-w-0 rounded-lg border border-zinc-300 bg-white px-3 text-base sm:text-sm" name="sort" defaultValue={params.sort}><option value="newest">Recently updated</option><option value="price-asc">Price: low to high</option><option value="price-desc">Price: high to low</option><option value="name">Name A–Z</option><option value="stock">Most available</option><option value="oldest">Oldest updated</option></select>
  ...
  return <main className="mx-auto max-w-7xl px-4 py-7 sm:px-6 sm:py-10 lg:px-8">
    ...
    {adminTestProducts.length && Number(params.page || 1) === 1 ? <p className="mb-3 rounded-lg border border-violet-200 bg-violet-50 px-4 py-3 text-sm font-semibold text-violet-900">Testing access: the R1 payment test product is shown first and remains hidden from other customers.</p> : null}
