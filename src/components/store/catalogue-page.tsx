...
export async function CataloguePage({ params, heading = "Shop technology" }: { params: CatalogueParams; heading?: string }) {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-7 sm:px-6 sm:py-10 lg:px-8">
    ...
    {result.products.length ? <div className="grid grid-cols-2 gap-2.5 sm:gap-4 lg:grid-cols-4">{result.products.map(product => <ProductCard key={product.id} product={product} />)}</div> : <div className="rounded-lg border border-dashed border-slate-300 px-4 py-16 text-center"><h2 className="text-xl font-semibold">No products found</h2><p className="mt-2 text-slate-600">Try changing your search or filters.</p></div>}
