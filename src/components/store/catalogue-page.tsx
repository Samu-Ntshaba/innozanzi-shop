...
export async function CataloguePage({ params, heading = "Shop technology" }: { params: CatalogueParams; heading?: string }) {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-7 sm:px-6 sm:py-10 lg:px-8">
    ...
    {adminTestProducts.length && Number(params.page || 1) === 1 ? <p className="mb-3 rounded-lg border border-violet-200 bg-violet-50 px-4 py-3 text-sm font-semibold text-violet-900">Administrator view: the R1 payment test product is shown first and is hidden from customers.</p> : null}
