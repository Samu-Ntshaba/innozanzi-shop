...
export function DeliveryAddressFields({ addresses = [], mapsEnabled, name = "", phone = "" }: { addresses?: SavedAddress[]; mapsEnabled: boolean; name?: string; phone?: string }) {
  const usable = addresses.filter(a => !mapsEnabled || a.googlePlaceId);
  const [selected, setSelected] = useState(usable[0]?.id ?? "");
  const [address, setAddress] = useState<DeliveryAddress>({ recipient: name, phone, line1: "", line2: "", suburb: "", city: "", province: "Gauteng", postalCode: "" });
  ...
  async function choose(suggestion: Suggestion) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error);
      if (current !== generation.current) return;
      setAddress(a => ({ ...a, ...result.address })); setProof(result.proof); setQuery(suggestion.text);
  ...
  return <div className="mt-5 space-y-4">
    ...
    {chosen ? <div className="rounded-lg bg-slate-50 p-4 text-sm leading-6"><strong>{chosen.recipient}</strong><p>{chosen.line1}{chosen.line2 ? `, ${chosen.line2}` : ""}</p><p>{chosen.suburb} {chosen.city}, {chosen.province}, {chosen.postalCode}</p><p>{chosen.phone}</p><Link href="/account/addresses" className="text-sky-700 underline">Manage saved addresses</Link></div> : <>
      ...
      <div className="grid gap-4 sm:grid-cols-2">{fields.map(([field, label, autocomplete]) => <label key={field} className="text-sm font-semibold">{label}<input className={inputClass} name={field} value={address[field]} autoComplete={autocomplete} type={field === "phone" ? "tel" : "text"} required={!["line2", "suburb"].includes(field)} maxLength={field === "postalCode" ? 4 : field === "phone" ? 40 : 180} pattern={field === "postalCode" ? "[0-9]{4}" : undefined} inputMode={field === "postalCode" ? "numeric" : undefined} readOnly={mapsEnabled && ["line1", "city", "suburb", "postalCode"].includes(field)} onChange={e => setAddress(a => ({ ...a, [field]: e.target.value }))}/></label>)}
        <label className="text-sm font-semibold">Province<select className={inputClass} name={mapsEnabled ? undefined : "province"} value={address.province} disabled={mapsEnabled} onChange={e => setAddress(a => ({ ...a, province: e.target.value as DeliveryAddress["province"] }))}>{provinces.map(p => <option key={p}>{p}</option>)}</select>{mapsEnabled && <input type="hidden" name="province" value={address.province}/>}</label>
      </div>
