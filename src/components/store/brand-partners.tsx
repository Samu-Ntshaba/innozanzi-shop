...
const partners=[
  {name:"WINX",logo:"/marketing/brands/winx-on-white.png"},
...
export function BrandPartners(){
  return <section aria-labelledby="brand-partners-title" className="border-y border-slate-200 bg-white">
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">{partners.map(partner=><Link aria-label={`Browse ${partner.name} products`} className="group flex min-h-24 items-center justify-center rounded-xl border border-slate-200 bg-white p-3 transition hover:-translate-y-0.5 hover:border-sky-400 hover:shadow-md" href={`/shop?brand=${encodeURIComponent(partner.name)}`} key={partner.name}><Image alt={`${partner.name} logo`} className="h-16 w-full object-contain opacity-100" height={96} src={partner.logo} width={160}/></Link>)}</div>
    </div>
  </section>;
}
