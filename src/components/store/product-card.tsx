  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white transition hover:border-sky-300 hover:shadow-xl">
      <Link href={`/products/${product.slug}`} className="relative block aspect-square overflow-hidden bg-white">
        {image ? <Image src={image.path} alt={image.altText ?? product.name} fill sizes="(max-width: 768px) 50vw, 25vw" className="object-contain p-5 transition duration-300 group-hover:scale-105" /> : <div className="grid h-full place-items-center bg-slate-50 text-xs text-slate-400">Product image coming soon</div>}
      ...
      <div className="flex flex-1 flex-col border-t border-slate-100 p-4">
        ...
        <div className="mt-auto pt-4">
          ...
          <div className="flex items-end justify-between gap-3">
            <div><p className="text-xl font-black tracking-tight text-slate-950">{formatZar(onSale ? product.salePrice!.toString() : product.regularPrice.toString())}</p><p className="text-[11px] text-slate-500">VAT included</p></div>
