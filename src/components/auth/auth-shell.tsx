...
export function AuthShell({eyebrow,title,description,children,footer}:{eyebrow:string;title:string;description:string;children:ReactNode;footer:ReactNode}){
  return <main className="min-h-screen lg:grid lg:grid-cols-[minmax(360px,.82fr)_minmax(520px,1.18fr)]">
    <aside className="relative hidden overflow-hidden bg-[#071b33] px-10 py-12 text-white lg:flex lg:min-h-screen lg:flex-col xl:px-16">
      ...
      <div className="relative z-10 my-auto max-w-md py-16"><p className="text-xs font-bold uppercase tracking-[.2em] text-sky-300">Innozanzi Shop</p><h2 className="mt-4 text-4xl font-black leading-tight xl:text-5xl">Your technology, in one account.</h2><p className="mt-5 text-base leading-7 text-slate-300">Shop, pay securely and follow your delivery without losing your place.</p><div className="mt-9 space-y-4"><Benefit icon={<CheckCircle2/>} text="Keep orders and PC projects together"/><Benefit icon={<ShieldCheck/>} text="Secure checkout and account access"/><Benefit icon={<Headphones/>} text="Help when you need it"/></div></div>
      <p className="relative z-10 text-xs text-slate-400">Innozanzi (Pty) Ltd · Technology people who listen</p>
    </aside>
