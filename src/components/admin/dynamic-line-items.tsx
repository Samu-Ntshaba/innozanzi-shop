...
export function DynamicLineItems({kind,maxRows=12}:{kind:Kind;maxRows?:number}){
  ...
  return <div className="space-y-3">
    <div className={`hidden gap-2 px-3 text-[10px] font-bold uppercase tracking-wider text-slate-500 sm:grid ${kind==="invoice"?"grid-cols-[minmax(0,1fr)_90px_140px_110px_42px]":priced?"grid-cols-[minmax(0,1fr)_90px_140px_42px]":"grid-cols-[minmax(0,1fr)_110px_42px]"}`}>
      <span>Description</span><span>Quantity</span>{priced?<span>Unit price</span>:null}{kind==="invoice"?<span>VAT %</span>:null}<span/>
    </div>
    {rows.map((row,index)=><div className={`grid gap-3 border border-slate-300 bg-slate-50/60 p-3 ${kind==="invoice"?"sm:grid-cols-[minmax(0,1fr)_90px_140px_110px_42px]":priced?"sm:grid-cols-[minmax(0,1fr)_90px_140px_42px]":"sm:grid-cols-[minmax(0,1fr)_110px_42px]"} sm:items-center`} key={row.id}>
