...
export function OrderSupplierSetup({orderId,suppliers}:{orderId:string;suppliers:Array<{id:string;companyName:string}>}){
  ...
  return <div className="space-y-3">{suppliers.map(supplier=><form action={saveOrderProcurement} className="grid gap-3 rounded border border-amber-200 bg-amber-50 p-4" key={supplier.id}>
    ...
    <label>Supplier order/reference number<input className={`${inputClass} mt-1 w-full`} name="supplierReference" required/></label>
