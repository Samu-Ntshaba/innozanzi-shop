...
export function BrandLogo({ className, priority = false, variant = "default", href = "/" }: { className?: string; priority?: boolean; variant?: "default" | "footer"; href?: string }) {
  if (variant === "footer") {
    return (
      <Link href={href} aria-label={`${brand.shopName} home`} className={cn("inline-flex shrink-0", className)}>
