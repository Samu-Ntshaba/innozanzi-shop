...
export async function POST(request: Request) {
    const blocked = browserMutationGuard(request);
    if (blocked)
        return blocked;
    ...
    const metadata = usage.metadata && typeof usage.metadata === "object" && !Array.isArray(usage.metadata) ? usage.metadata as Record<string, unknown> : {}, allowed = Array.isArray(metadata.recommendedProductIds) ? metadata.recommendedProductIds.filter((id): id is string => typeof id === "string") : [];
    if (!parsed.data.productIds.every(id => allowed.includes(id)) || parsed.data.kind !== usage.intentType)
        return NextResponse.json({ error: "This selection does not belong to the verified recommendation." }, { status: 400 });
    const ids = [...new Set(parsed.data.productIds)], products = await prisma.supplierCatalogueProduct.findMany({ where: {
            ...{ id: { in: ids }, active: true, availability: "IN_STOCK", stock: { gt: 0 }, costPrice: { gt: 0 } },
            ...await sellableSupplierWhere()
        } });
    if (products.length !== ids.length)
        return NextResponse.json({ error: "Price or stock changed. Please request a fresh recommendation." }, { status: 409 });
    if (parsed.data.kind === "PC_BUILD") {
        const selected: Record<string, (typeof products)[number]> = {};
        for (const product of products) {
            const text = `${product.categoryPath ?? ""} ${product.name}`.toLowerCase(), key = text.includes("motherboard") ? "motherboard" : text.includes("cpu/") || text.includes("processor") ? "cpu" : text.includes("memory") ? "memory" : text.includes("solid state") || text.includes("hard disk") || text.includes("storage") ? "storage" : text.includes("graphics") ? "graphics" : text.includes("power suppl") ? "power" : text.includes("chassis") || text.includes("case") ? "case" : undefined;
            if (key)
                selected[key] = product;
        }
        for (const [key, product] of Object.entries(selected)) {
            if (pcPartCompatibility(key, product, selected).kind === "bad")
    ...
    await prisma.$transaction(async (tx) => {
        ...
        for (const product of products)
            await tx.supplierCartItem.upsert({ where: { cartId_supplierId_supplierProductId: { cartId: cart.id, supplierId: product.supplierId, supplierProductId: product.supplierProductId } }, create: { cartId: cart.id, supplierId: product.supplierId, supplierProductId: product.supplierProductId, supplierSku: product.supplierSku, quantity: 1 }, update: { quantity: 1 } });
        await tx.aIUsage.update({ where: { id: usage.id }, data: { cartCreated: true } });
        await tx.recommendationEvent.createMany({ data: products.map(product => ({ userId: identity.userId, sessionId: identity.userId ? `user:${identity.userId}` : identity.anonymousSessionId!, eventType: "AI_ADD_TO_CART", entityType: "SUPPLIER_PRODUCT", entityId: product.id, category: product.category, brand: product.brand, recommendationId: parsed.data.recommendationId, context: "ai-shopping" })) });
    });
    return NextResponse.json({ ok: true, redirect: "/cart?status=ai-added" });
}
