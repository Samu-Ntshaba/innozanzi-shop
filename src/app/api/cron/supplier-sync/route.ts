...
export async function POST(request:Request){if(!valid(request))return NextResponse.json({error:"Unauthorized"},{status:401});const active=await prisma.supplierSyncRun.findFirst({where:{status:"RUNNING",startedAt:{gt:new Date(Date.now()-30*60_000)}}});if(active)return NextResponse.json({status:"already-running",runId:active.id},{status:202});const mode=new URL(request.url).searchParams.get("mode")?.toUpperCase()==="FULL"?"FULL":"INCREMENTAL";try{const results=await syncAllSuppliers(mode);return NextResponse.json({mode,results},{status:results.some(r=>r.status==="FAILED")?500:200})}catch{return NextResponse.json({error:"Supplier sync failed"},{status:500})}}
