...
export async function GET(_: Request, { params }: { params: Promise<{ invoiceNumber: string }> }) {
  ...
  const invoice=await prisma.invoice.findUnique({where:{invoiceNumber:(await params).invoiceNumber},include:{quotation:{include:{items:true}}}});
  ...
  const pdf=commercialPdf({title:"TAX INVOICE",number:invoice.invoiceNumber,customer:invoice.companyName??invoice.customerName,email:invoice.customerEmail,issueDate:invoice.issuedAt??invoice.createdAt,dueDate:invoice.dueAt,reference:invoice.quotation?.quotationNumber,lines:invoice.quotation?.items.map(item=>({description:item.productName,quantity:item.quantity,unitPrice:zar(item.unitPrice),total:zar(item.lineTotal)}))??[],subtotal:zar(invoice.subtotal),vat:zar(invoice.vatTotal),total:zar(invoice.grandTotal),notes:invoice.notes});
