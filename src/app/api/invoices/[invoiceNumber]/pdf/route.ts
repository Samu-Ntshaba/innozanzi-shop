...
export async function GET(_: Request, { params }: { params: Promise<{ invoiceNumber: string }> }) {
  ...
  const invoice=await prisma.invoice.findUnique({where:{invoiceNumber:(await params).invoiceNumber},include:{items:{orderBy:{sortOrder:"asc"}},quotation:{select:{quotationNumber:true}},order:{select:{orderNumber:true,userId:true}}}});
  if(!invoice)return new Response("Not found",{status:404});
  if((invoice.order?invoice.order.userId!==auth.user.id:invoice.customerEmail.toLowerCase()!==auth.user.email.toLowerCase())&&!hasPermission(auth.grants,"quotations.manage",auth.isSuperAdministrator))return new Response("Forbidden",{status:403});
  ...
  return new Response(new Uint8Array(pdf),{headers:{"content-type":"application/pdf","cache-control":"private, no-store","content-disposition":`inline; filename=\"${invoice.invoiceNumber}.pdf\"`}});
}
