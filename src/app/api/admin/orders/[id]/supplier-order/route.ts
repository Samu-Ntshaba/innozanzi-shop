...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const pdf=await supplierOrderRequestPdf({orderNumber:order.orderNumber,createdAt:order.createdAt,customer:order.companyName??order.email,recipient:address?.recipient,phone:address?.phone??order.phone??undefined,deliveryAddress:address?[address.line1,address.line2,address.suburb,address.city,address.province,address.postalCode,address.countryCode].filter(Boolean).join(", "):undefined,deliveryInstructions:order.customerNotes??undefined,items:supplierItems.map(item=>({name:item.productName,sku:item.supplierSku??item.sku,quantity:item.quantity,costPrice:item.costPrice?.toString()??null,supplierName:item.supplierId?names.get(item.supplierId)??"Supplier":"Supplier"}))});
