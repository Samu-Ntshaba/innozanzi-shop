...
function destination(caseId: string, status: string, message?: string) {
  const url = new URL(`/account/partner/sales/${encodeURIComponent(caseId)}`, publicSiteUrl());
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === "revision") {
      ...
      return NextResponse.redirect(destination(id, "revision-requested"), 303);
    }
    const result = await sendPartnerQuotation(id, actor);
    const url = destination(id, "sent");
