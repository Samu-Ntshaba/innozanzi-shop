...
export default async function AccountLayout({ children }: { children: React.ReactNode }) {
  ...
  const showPartnerSales=Boolean(participation?._count.partnerships && (await partnerSalesSettings()).enabled);
    ...
    <div className="account-shell flex min-h-screen flex-col overflow-x-clip bg-slate-50 text-slate-950">
      ...
      <div className="mx-auto grid w-full max-w-[1440px] flex-1 lg:grid-cols-[240px_minmax(0,1fr)]">
        <aside className="border-b border-slate-200 bg-white lg:sticky lg:top-28 lg:self-start lg:border-b-0 lg:border-r lg:pb-4">
          ...
          <AccountNav showAdmin={showAdmin} showPartnership={showPartnership} showPartnerSales={showPartnerSales} />
