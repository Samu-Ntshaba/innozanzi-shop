...
export default async function TrackOrder({ params, searchParams }: { params: Promise<{ orderNumber: string }>; searchParams: Promise<{ quotation?: string; payment?: string }> }) {
  ...
  return <main className="mx-auto min-w-0 max-w-5xl px-4 py-7 sm:py-10">
    ...
    <div className="mt-2 flex min-w-0 flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><h1 className="min-w-0 break-all text-2xl font-black sm:break-words sm:text-3xl">Order {order.orderNumber}</h1><div className="grid w-full grid-cols-1 gap-2 min-[420px]:grid-cols-2 sm:flex sm:w-auto"> <Link className="inline-flex min-h-11 items-center justify-center rounded-lg border px-3 text-center text-sm font-bold text-sky-700" href="/account/orders">All orders</Link>{!unpaid ? <><a className="inline-flex min-h-11 items-center justify-center rounded-lg border px-3 text-center text-sm font-bold text-sky-700" href={`/api/orders/${order.orderNumber}/delivery-note`} target="_blank">Delivery note</a><Link className="inline-flex min-h-11 items-center justify-center rounded-lg bg-sky-700 px-3 text-center text-sm font-bold text-white min-[420px]:col-span-2 sm:col-span-1" href={`/account/returns/new?order=${order.orderNumber}`}>Get product help</Link></> : null}</div></div>
