...
export default async function TrackOrder({ params, searchParams }: { params: Promise<{ orderNumber: string }>; searchParams: Promise<{ quotation?: string; payment?: string }> }) {
  ...
  const [order, completionDays, eftSettings] = await Promise.all([
    prisma.order.findFirst({ where: { userId: ctx.user.id, ...(orderNumber === "by-quotation" && query.quotation ? { convertedQuotation: { quotationNumber: query.quotation } } : { orderNumber }) }, include: { items: true, deliveryEvents: { orderBy: { occurredAt: "asc" } }, shipments: { orderBy: { createdAt: "desc" } }, procurements: { select: { orderedAt: true, confirmedAt: true }, orderBy: { createdAt: "asc" } }, returnCases: { select: { id: true, status: true } } } }),
  ...
  return <main className="mx-auto min-w-0 max-w-5xl px-4 py-7 sm:py-10">
    ...
    <div className="mt-2 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><h1 className="min-w-0 break-words text-2xl font-black sm:text-3xl">Order {order.orderNumber}</h1><div className="flex"><Link className="inline-flex min-h-11 items-center justify-center rounded-lg border px-4 text-center text-sm font-bold text-sky-700" href="/account/orders">All orders</Link>{!unpaid ? <><a className="ml-2 inline-flex min-h-11 items-center justify-center rounded-lg border px-3 text-center text-sm font-bold text-sky-700" href={`/api/orders/${order.orderNumber}/delivery-note`} target="_blank">Delivery note</a><Link className="ml-2 inline-flex min-h-11 items-center justify-center rounded-lg bg-sky-700 px-3 text-center text-sm font-bold text-white" href={`/account/returns/new?order=${order.orderNumber}`}>Get product help</Link></> : null}</div></div>
    {unpaid ? <OrderPaymentPanel order={order} notice={query.payment} ozowAvailable={gatewayConfigured("OZOW")} eftSettings={eftSettings}/> : <section className="mt-6 overflow-hidden rounded-xl border border-slate-200 bg-white p-4 sm:p-6"><h2 className="mb-5 text-xl font-black">Order progress</h2><OrderProgress status={order.status} details={{ placedAt: order.placedAt, estimatedDeliveryAt: shipment?.estimatedDeliveryAt, deliveredAt: shipment?.deliveredAt, returnWindowEndsAt, supplierOrderedAt, supplierConfirmedAt, events: order.deliveryEvents }}/></section>}
    ...
    <div className="mt-6 grid min-w-0 gap-5 md:grid-cols-[minmax(0,1fr)_minmax(16rem,.8fr)]">
      {!unpaid ? <section className="min-w-0 rounded-xl border bg-white p-4 sm:p-5"><h2 className="font-bold">Verified updates</h2><div className="mt-5">{order.deliveryEvents.map((event, index) => <div className="relative border-l-2 border-sky-500 pb-7 pl-5 last:pb-0" key={event.id}><span className="absolute -left-[7px] top-0 size-3 rounded-full bg-sky-600"/><p className="font-semibold">{event.status.replaceAll("_", " ")}</p><p className="text-xs text-slate-500">{event.occurredAt.toLocaleString("en-ZA")}</p>{event.publicNote ? <p className="mt-1 text-sm leading-6 text-slate-700">{event.publicNote}</p> : null}{index === order.deliveryEvents.length - 1 ? <span className="mt-2 inline-block rounded bg-sky-50 px-2 py-1 text-xs font-bold text-sky-800">Latest update</span> : null}</div>)}</div></section> : <section className="rounded-xl border bg-slate-50 p-5 text-sm text-slate-700"><h2 className="font-bold text-slate-950">What happens next?</h2><p className="mt-2 leading-6">Complete an Ozow payment or submit EFT proof. Products are only reserved and fulfilment only begins after payment verification.</p></section>}
