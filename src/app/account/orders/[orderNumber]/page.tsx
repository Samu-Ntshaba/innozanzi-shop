...
export default async function TrackOrder({ params, searchParams }: { params: Promise<{ orderNumber: string }>; searchParams: Promise<{ quotation?: string; payment?: string }> }) {
  ...
  const [order, completionDays, eftSettings] = await Promise.all([
    prisma.order.findFirst({ where: { userId: ctx.user.id, OR: [{ paymentStatus: { in: ["PAID", "REFUNDED", "PARTIALLY_REFUNDED"] } }, { status: "CANCELLED" }], ...(orderNumber === "by-quotation" && query.quotation ? { convertedQuotation: { quotationNumber: query.quotation } } : { orderNumber }) }, include: { items: true, deliveryEvents: { orderBy: { occurredAt: "asc" } }, shipments: { orderBy: { createdAt: "desc" } }, procurements: { select: { orderedAt: true, confirmedAt: true }, orderBy: { createdAt: "asc" } }, returnCases: { select: { id: true, status: true } } } }),
