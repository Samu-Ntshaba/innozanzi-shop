...
export async function GET() {
    const [{ siteUrl: base, businessName }, local, supplier] = await Promise.all([
        ...
        prisma.supplierCatalogueProduct.findMany({ where: {
                ...{ active: true, costPrice: { gt: 0 }, images: { isEmpty: false } },
                ...await sellableSupplierWhere()
            }, select: { id: true, name: true, slug: true, supplierSku: true, manufacturerSku: true, barcode: true, currency: true, costPrice: true, recommendedRetail: true, promotionalPrice: true, promotionStartsAt: true, promotionEndsAt: true, shortDescription: true, description: true, availability: true, stock: true, brand: true, category: true, images: true } }),
    ]);
    ...
    const supplierItems = await Promise.all(supplier.map(async (product) => { const retail = await supplierRetailPrice({ costPrice: product.costPrice!, recommendedRetail: product.recommendedRetail, promotionalPrice: product.promotionalPrice, promotionStartsAt: product.promotionStartsAt, promotionEndsAt: product.promotionEndsAt, special: isDailySpecial(product.id), productKey:`SUPPLIER:${product.id}` }), price = retail.salePrice ?? retail.regularPrice; return item({ id: `supplier-${product.id}`, title: product.name, description: plain(product.shortDescription ?? product.description), link: `${base}/supplier-products/${product.slug}`, image: new URL(product.images[0], base).toString(), availability: product.stock > 0 && product.availability === "IN_STOCK" ? "in_stock" : "out_of_stock", price: price.toFixed(2), brand: product.brand, mpn: product.manufacturerSku, gtin: product.barcode, category: product.category }); }));
