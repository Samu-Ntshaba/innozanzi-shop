...
export default async function PortalLayout({children}:{children:React.ReactNode}){
  ...
  return <div className="min-h-screen bg-[#f1f6fa] text-slate-950">
    <header className="sticky top-0 z-40 border-b border-sky-400/25 bg-[#071b33] text-white shadow-sm">
      <div className="flex h-16 items-center justify-between gap-4 px-4 sm:px-6">
        <div className="flex min-w-0 items-center gap-4"><BrandLogo variant="footer" className="w-32 sm:w-36" priority/><span className="hidden h-7 border-l border-white/25 sm:block"/><div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-[.2em] text-sky-300">Client Portal</p><p className="truncate text-sm font-semibold text-white">{organisation}</p></div></div>
