...
}) => <div className="flex gap-2.5"><Icon className="mt-0.5 size-5 shrink-0 text-sky-700"/><span><strong className="block">{title}</strong><small className="text-slate-500">{children}</small></span></div>;
export default async function SupplierProductPage({ params, searchParams }: {
    ...
    const [product, context, query] = await Promise.all([prisma.supplierCatalogueProduct.findFirst({ where: {
                ...{ slug: (await params).slug, active: true },
                ...await sellableSupplierWhere()
            }, include: { supplier: { select: { companyName: true } }, reviews: { where: { status: "APPROVED" }, orderBy: { createdAt: "desc" }, take: 20, include: { user: { select: { name: true } } } } } }), getAuthContext(), searchParams]);
    if (!product)
        notFound();
    ...
    <div className="mt-4 grid items-start gap-6 sm:mt-6 sm:gap-8 lg:grid-cols-[minmax(0,1.08fr)_minmax(380px,.92fr)] lg:gap-12"><ProductGallery images={product.images} name={product.name}/><section className="lg:sticky lg:top-28">
      ...
      <div className="mt-5 grid grid-cols-2 gap-4 text-sm"><Benefit icon={Truck} title="Delivery coverage"><DeliveryBenefit/></Benefit><Benefit icon={ShieldCheck} title="Warranty">{product.warranty ?? "Supplier backed"}</Benefit><Benefit icon={CreditCard} title="Secure payment">Pay securely with Ozow</Benefit><Benefit icon={RotateCcw} title="Returns support"><Link className="text-sky-700 underline" href="/returns-policy">View policy</Link></Benefit></div><ProductShare title={product.name} path={`/supplier-products/${product.slug}`}/>
    </section></div>
    <section className="mt-14 border-t border-slate-200 pt-10"><h2 className="text-2xl font-bold text-slate-950">About this product</h2>{product.description ? <div className="prose prose-slate mt-4 max-w-4xl leading-8" dangerouslySetInnerHTML={{ __html: safeSupplierHtml(product.description) }}/> : <p className="mt-4 leading-8 text-slate-700">Product information is currently unavailable.</p>}</section>
