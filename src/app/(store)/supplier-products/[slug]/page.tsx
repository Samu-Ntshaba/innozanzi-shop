...
export async function generateMetadata({ params }: {
    ...
    const product = await prisma.supplierCatalogueProduct.findFirst({ where: {
            ...{ slug: (await params).slug, active: true },
...
export default async function SupplierProductPage({ params, searchParams }: {
    ...
    const [product, context, query] = await Promise.all([prisma.supplierCatalogueProduct.findFirst({ where: {
                ...{ slug: (await params).slug, active: true },
                ...await sellableSupplierWhere()
    ...
    const verifiedFacts = [{ label: "Brand", value: product.brand }, { label: "Category", value: product.category }, { label: "Product code", value: product.supplierSku }, { label: "Manufacturer code", value: product.manufacturerSku && product.manufacturerSku !== product.supplierSku ? product.manufacturerSku : null }].filter((fact): fact is { label: string; value: string } => Boolean(fact.value));
    return <main className="bg-slate-50 pb-20"><BehaviourSignal signal={{ eventType: "VIEW", entityType: "SUPPLIER_PRODUCT", entityId: product.id, category: product.category ?? undefined, brand: product.brand ?? undefined, price: current ? Number(current) : undefined, context: "product" }}/>{query.recommendation ? <BehaviourSignal signal={{ eventType: "RECOMMENDATION_CLICK", entityType: "SUPPLIER_PRODUCT", entityId: product.id, recommendationId: query.recommendation, category: product.category ?? undefined, brand: product.brand ?? undefined, context: "product" }}/> : null}<div className="mx-auto max-w-7xl px-4 pt-5 sm:px-6 lg:px-8">
    <nav aria-label="Breadcrumb" className="flex min-w-0 items-center gap-1.5 overflow-hidden text-sm text-slate-500"><Link className="shrink-0 hover:text-sky-700" href="/shop">Shop</Link><ChevronRight className="size-3.5 shrink-0"/><Link className="min-w-0 truncate hover:text-sky-700" href={`/categories/${encodeURIComponent(product.category ?? "Catalogue")}`}>{product.category ?? "Catalogue"}</Link><ChevronRight className="size-3.5 shrink-0"/><span className="hidden truncate text-slate-700 sm:block">{product.name}</span></nav>
    <div className="mt-5 grid items-start gap-7 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 lg:grid-cols-[minmax(0,1.08fr)_minmax(380px,.92fr)] lg:gap-10 lg:p-8"><ProductGallery images={product.images} name={product.name}/><section className="rounded-2xl bg-slate-50 p-5 sm:p-7 lg:sticky lg:top-28">
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">{current ? <><div className="flex flex-wrap items-end gap-3"><p className="text-4xl font-black tracking-tight text-slate-950">{formatZar(current)}</p>{price?.salePrice ? <p className="pb-1 text-lg text-slate-400 line-through">{formatZar(price.regularPrice)}</p> : null}</div>{saving ? <p className="mt-1 text-sm font-semibold text-emerald-700">Daily deal — save {formatZar(saving)}</p> : null}</> : <p className="font-semibold text-amber-800">Temporarily unavailable for online purchase.</p>}</div>
      <div className={`mt-4 inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm font-semibold ${product.stock > 0 ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}><span className={`grid size-5 place-items-center rounded-full ${product.stock > 0 ? "bg-emerald-600 text-white" : "bg-amber-500 text-white"}`}><Check className="size-3.5"/></span>{product.stock > 0 ? `${product.stock} in stock and ready to order` : "Currently unavailable"}</div>
      ...
      {current && product.stock > 0 ? <form action={addSupplierCartItemAction} className="mt-6 grid grid-cols-[88px_1fr] gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><input type="hidden" name="productId" value={product.id}/><input aria-label="Quantity" className="h-13 rounded-xl border border-slate-300 bg-white px-3 text-center" name="quantity" type="number" min="1" max={product.stock} defaultValue="1"/><button className="h-13 rounded-xl bg-sky-700 px-6 text-base font-black text-white shadow-sm transition hover:bg-sky-800">Add to cart</button></form> : null}
      <div className="mt-5 grid gap-2.5 text-sm sm:grid-cols-2"><Benefit icon={Truck} title="Delivery coverage"><DeliveryBenefit/></Benefit><Benefit icon={ShieldCheck} title="Warranty">{product.warranty ?? "Supplier backed"}</Benefit><Benefit icon={CreditCard} title="Secure payment">Pay securely with PayFast or Ozow</Benefit><Benefit icon={RotateCcw} title="Returns support"><Link className="text-sky-700 underline" href="/returns-policy">View policy</Link></Benefit></div><ProductShare title={product.name} path={`/supplier-products/${product.slug}`}/>
    </section></div>
    <nav aria-label="Product information" className="mt-6 flex gap-1 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm"><a className="whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-sky-50 hover:text-sky-800" href="#overview">Overview</a>{specs.length ? <a className="whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-sky-50 hover:text-sky-800" href="#specifications">Specifications</a> : null}<a className="whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-sky-50 hover:text-sky-800" href="#reviews">Reviews ({product.reviews.length})</a></nav>
    <section id="overview" className="mt-6 scroll-mt-32 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><p className="text-xs font-black uppercase tracking-[.18em] text-sky-700">Product details</p><h2 className="mt-2 text-2xl font-black text-slate-950">About this product</h2>{product.description ? <div className="prose prose-slate mt-4 max-w-4xl leading-8" dangerouslySetInnerHTML={{ __html: safeSupplierHtml(product.description) }}/> : <div className="mt-4 max-w-4xl"><p className="leading-8 text-slate-700"><strong>{product.name}</strong>{product.brand ? ` from ${product.brand}` : ""} is available in our {product.category?.toLowerCase() ?? "technology"} range. The information below is verified from the current catalogue listing.</p><dl className="mt-5 grid overflow-hidden rounded-2xl border border-slate-200 sm:grid-cols-2">{verifiedFacts.map((fact, index) => <div className={`px-4 py-3.5 ${index % 2 === 0 ? "bg-slate-50" : "bg-white"}`} key={fact.label}><dt className="text-xs font-bold uppercase tracking-wide text-slate-500">{fact.label}</dt><dd className="mt-1 font-semibold text-slate-900">{fact.value}</dd></div>)}</dl><p className="mt-5 rounded-xl border border-sky-100 bg-sky-50 p-4 text-sm leading-6 text-sky-950">A longer manufacturer description was not included in the live catalogue feed. Use the product name, images and verified codes above when checking compatibility, or <Link className="font-bold underline" href="/contact">ask us to confirm the full specification</Link> before ordering.</p></div>}</section>
    {specs.length ? <section id="specifications" className="mt-6 scroll-mt-32 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><p className="text-xs font-black uppercase tracking-[.18em] text-sky-700">Product details</p><h2 className="mt-2 text-2xl font-black text-slate-950">Technical specifications</h2><dl className="mt-5 overflow-hidden rounded-2xl border border-slate-200">{specs.map(([name, value], index) => <div className={`grid gap-1 px-4 py-3.5 sm:grid-cols-[minmax(180px,.4fr)_1fr] sm:gap-6 ${index % 2 ? "bg-slate-50" : "bg-white"}`} key={name}><dt className="font-semibold capitalize text-slate-800">{name.replaceAll("-", " ")}</dt><dd className="break-words text-slate-600">{typeof value === "object" ? JSON.stringify(value) : String(value)}</dd></div>)}</dl></section> : null}
    <div className="mt-6"><ProductReviews productId={product.id} sourceType="SUPPLIER" path={`/supplier-products/${product.slug}`} reviews={product.reviews} signedIn={Boolean(context)} submitted={query.review === "submitted"}/></div>
