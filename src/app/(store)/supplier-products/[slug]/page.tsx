...
export default async function SupplierProductPage({ params, searchParams }: {
    ...
    <div className="mt-5 grid items-start gap-7 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 lg:grid-cols-[minmax(0,1.08fr)_minmax(380px,.92fr)] lg:gap-10 lg:p-8"><ProductGallery images={product.images} name={product.name}/><section className="rounded-2xl bg-slate-50 p-5 sm:p-7 lg:sticky lg:top-28">
      ...
      <div className="mt-5 grid gap-2.5 text-sm sm:grid-cols-2"><Benefit icon={Truck} title="Delivery coverage"><DeliveryBenefit/></Benefit><Benefit icon={ShieldCheck} title="Warranty">{product.warranty ?? "Supplier backed"}</Benefit><Benefit icon={CreditCard} title="Secure payment">Pay securely with Ozow</Benefit><Benefit icon={RotateCcw} title="Returns support"><Link className="text-sky-700 underline" href="/returns-policy">View policy</Link></Benefit></div><ProductShare title={product.name} path={`/supplier-products/${product.slug}`}/>
    </section></div>
