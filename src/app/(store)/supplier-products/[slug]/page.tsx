...
export default async function SupplierProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="bg-white pb-14"><div className="mx-auto max-w-7xl px-4 pt-5 sm:px-6 lg:px-8">
    ...
    <div className="mt-6 grid items-start gap-8 lg:grid-cols-[minmax(0,1.08fr)_minmax(380px,.92fr)] lg:gap-12"><ProductGallery images={product.images} name={product.name}/><section className="lg:sticky lg:top-28">
      ...
      <div className="mt-5 grid grid-cols-2 gap-4 text-sm"><Benefit icon={Truck} title="Nationwide delivery">Tracked fulfilment</Benefit><Benefit icon={ShieldCheck} title="Warranty">{product.warranty ?? "Supplier backed"}</Benefit><Benefit icon={CreditCard} title="Secure payment">Paystack or EFT</Benefit><Benefit icon={RotateCcw} title="Returns support"><Link className="text-sky-700 underline" href="/returns-policy">View policy</Link></Benefit></div><ProductShare title={product.name} path={`/supplier-products/${product.slug}`}/>
    </section></div>
    <div className="mt-14 grid gap-10 border-t border-slate-200 pt-10 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,.75fr)]"><section><h2 className="text-2xl font-bold text-slate-950">About this product</h2>{product.description ? <div className="prose prose-slate mt-4 max-w-none leading-8" dangerouslySetInnerHTML={{ __html: safeSupplierHtml(product.description) }}/> : <p className="mt-4 leading-8 text-slate-700">A reliable technology product supplied and supported by Innozanzi.</p>}</section><aside className="rounded-2xl bg-[#071b33] p-6 text-white"><PackageCheck className="size-7 text-sky-300"/><h2 className="mt-4 text-xl font-bold">Live supplier availability</h2><p className="mt-2 text-sm leading-6 text-slate-300">Stock and pricing are checked again when your order is placed. You receive an order reference and can follow fulfilment from your account.</p></aside></div>
