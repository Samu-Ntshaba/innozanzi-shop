...
export default async function SupplierProductPage({ params, searchParams }: {
    ...
    <div className="mt-6 grid items-start gap-8 lg:grid-cols-[minmax(0,1.08fr)_minmax(380px,.92fr)] lg:gap-12"><ProductGallery images={product.images} name={product.name}/><section className="lg:sticky lg:top-28">
      ...
      <div className="mt-5 grid grid-cols-2 gap-4 text-sm"><Benefit icon={Truck} title="Nationwide delivery"><DeliveryBenefit/></Benefit><Benefit icon={ShieldCheck} title="Warranty">{product.warranty ?? "Supplier backed"}</Benefit><Benefit icon={CreditCard} title="Secure payment">Ozow or PayFast</Benefit><Benefit icon={RotateCcw} title="Returns support"><Link className="text-sky-700 underline" href="/returns-policy">View policy</Link></Benefit></div><ProductShare title={product.name} path={`/supplier-products/${product.slug}`}/>
    </section></div>
