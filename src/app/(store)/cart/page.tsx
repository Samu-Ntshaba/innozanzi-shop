...
export default async function CartPage({ searchParams }: { searchParams: Promise<{ error?: string; status?: string }> }) {
  ...
  let lines:Awaited<ReturnType<typeof resolveQuotationCart>>=[];let priceError="";
  try{lines=cart&&(items.length||supplierItems.length)?await resolveQuotationCart(cart,new Decimal(0)):[];}catch(error){priceError=error instanceof Error?error.message:"Please review your basket.";}
  const productTotal = lines.reduce((sum, line) => sum.plus(line.grossUnit.mul(line.quantity)), new Decimal(0));
  const delivery = productTotal.gt(0)&&productTotal.lt(settings.freeDeliveryThreshold)?new Decimal(settings.customerDelivery):new Decimal(0);
  const orderTotal = productTotal.plus(delivery);
  const supplierProducts = supplierItems.length?await prisma.supplierCatalogueProduct.findMany({ where: { OR: supplierItems.map(x => ({ supplierId: x.supplierId, supplierProductId: x.supplierProductId })),...await sellableSupplierWhere() }, select: { supplierId: true, supplierProductId: true, name: true, slug: true, images: true, availability: true,stock:true } }):[];
  return <main className="mx-auto max-w-7xl px-4 pb-28 pt-7 sm:px-6 sm:py-10 lg:px-8">
    ...
    {priceError?<p className="mt-4 rounded-lg bg-amber-50 p-4 text-sm text-amber-900">{priceError} Checkout is unavailable until the price and stock can be confirmed. You can remove items below.</p>:null}
    ...
    {params.status === "updated" ? <p className="mt-4 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-700">Cart quantity updated.</p> : null}
    {params.status === "removed" ? <p className="mt-4 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-700">Product removed from your cart.</p> : null}
