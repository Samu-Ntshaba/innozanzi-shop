  ...
  return <main className="min-h-screen bg-slate-50 pb-16">
    ...
    <CheckoutForm className="mx-auto grid max-w-7xl items-start gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_420px] lg:px-8">
      ...
      <aside className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:sticky lg:top-28">
        ...
        {paymentAmountError("PAYFAST",total) ? <p role="status" className="px-6 pt-4 text-sm text-amber-900">{paymentAmountError("PAYFAST",total)}</p> : null}
        <PaymentMethodSelector total={formatZar(total)} available={{PAYFAST:gatewayConfigured("PAYFAST")&&!paymentAmountError("PAYFAST",total),OZOW:gatewayConfigured("OZOW")}}/>
      </aside>
    </CheckoutForm>
  </main>;
}
