...
export default async function CheckoutPage({searchParams}:{searchParams:Promise<{coupon?:string}>}) {
  ...
  const addresses = await listAddresses(user.user.id);
  ...
  return <main className="min-h-screen bg-slate-50 pb-16">
    ...
    <form action="/checkout" method="get" className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 pt-5"><label className="text-sm font-semibold">Have a coupon?<input name="coupon" defaultValue={couponCode} maxLength={40} className="ml-2 rounded-lg border px-3 py-2" placeholder="Enter code"/></label><button className="rounded-lg border border-sky-700 px-4 py-2 text-sm text-sky-800">Apply coupon</button>{quote.coupon?<p className="text-sm text-emerald-700">{quote.coupon.code}: saving {formatZar(quote.coupon.discount)}</p>:null}</form>
    <CheckoutForm className="mx-auto grid max-w-7xl items-start gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_420px] lg:px-8">
      ...
      <div className="space-y-6">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          <DeliveryAddressFields addresses={addresses} mapsEnabled={mapsConfigured()} name={user.user.name ?? ""} phone={profile?.phone ?? ""}/>
