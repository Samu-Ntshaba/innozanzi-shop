...
export default async function BuildPcPage({searchParams}:{searchParams:Promise<{error?:string;project?:string}>}){
  ...
  return <PcBuilderWorkspace steps={steps} error={query.error} project={project?{id:project.id,name:project.name,buildType:project.buildType,items:project.items.map(item=>({stepKey:item.stepKey,productId:item.supplierProductId,purchased:Boolean(item.purchasedAt)}))}:undefined}/>;
}
