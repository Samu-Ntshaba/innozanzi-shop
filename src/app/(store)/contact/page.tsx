...
export const metadata: Metadata = {
  title: "Contact our business technology support team",
  description: "Contact Innozanzi for technology quotations, procurement advice, delivery, installation and ongoing support in South Africa.",
      ...
      <section className="border-b border-slate-200 bg-slate-50">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
          ...
          <div className="mt-3 grid gap-5 lg:grid-cols-[minmax(0,1fr)_25rem] lg:items-end">
            <div>
              <h1 className="max-w-3xl text-4xl font-bold tracking-tight text-slate-950 sm:text-5xl">How can we help your business?</h1>
            ...
            <p className="border-l-2 border-sky-600 pl-5 text-sm leading-6 text-slate-600">For a product quotation, add products to your quote list first. For orders, payments or technical support, use the form below.</p>
          </div>
        </div>
      </section>
        ...
        <div className="min-w-0">
          {submitted ? (
            ...
            <form action={submitHelpDeskTicket} className="grid gap-x-5 gap-y-5 sm:grid-cols-2">
              ...
              <label className={label}>Request type<select className={input} name="category" defaultValue="OTHER">{["QUOTATION", "ORDER", "PAYMENT", "PRODUCT", "TECHNICAL", "ACCOUNT", "OTHER"].map((item) => <option key={item} value={item}>{item.charAt(0) + item.slice(1).toLowerCase()}</option>)}</select></label>
              ...
              <label className={`${label} sm:col-span-2`}>Message <span className="text-sky-700">*</span><textarea className={`${input} min-h-40 py-3`} name="message" placeholder="Include a product, quotation or order reference if you have one." required /></label>
