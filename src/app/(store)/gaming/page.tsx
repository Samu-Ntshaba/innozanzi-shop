...
export default async function GamingPage({searchParams}:{searchParams:Promise<{search?:string;group?:string;brand?:string;page?:string}>}){
  ...
  return <main className="min-h-screen bg-white text-slate-950">
    ...
    <section className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
      ...
      {data.products.length?<div className="mt-5 grid gap-4 min-[480px]:grid-cols-2 lg:grid-cols-4">{data.products.map(product=><ProductCard product={product} key={product.id}/>)}</div>:<div className="py-20 text-center"><h2 className="font-bold">No matching products</h2><p className="mt-1 text-sm text-slate-500">Try another category, brand or search.</p></div>}
