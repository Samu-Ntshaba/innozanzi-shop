...
export default async function GamingPage({searchParams}:{searchParams:Promise<{search?:string;group?:string;brand?:string;page?:string}>}){
  ...
  return <main className="min-h-screen bg-white text-slate-950">
    <header className="border-b border-slate-200"><div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8"><h1 className="text-3xl font-black tracking-tight sm:text-4xl">Gaming</h1></div></header>
    <section className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
      ...
      {data.products.length?<div className="mt-5 grid grid-cols-2 gap-2.5 sm:grid-cols-3 sm:gap-3 lg:grid-cols-4 xl:grid-cols-5">{data.products.map(product=><ProductCard product={product} key={product.id}/>)}</div>:<div className="py-20 text-center"><h2 className="font-bold">No matching products</h2><p className="mt-1 text-sm text-slate-500">Try another category, brand or search.</p></div>}
