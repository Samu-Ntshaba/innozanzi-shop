...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="bg-white pb-14"><div className="mx-auto max-w-7xl px-4 pt-5 sm:px-6 lg:px-8">
    ...
    <div className="mt-14 grid gap-10 border-t border-slate-200 pt-10 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,.75fr)]"><section><h2 className="text-2xl font-bold text-slate-950">About this product</h2><div className="mt-4 whitespace-pre-line leading-8 text-slate-700">{product.description ?? product.shortDescription ?? "A reliable technology product supplied and supported by Innozanzi."}</div></section><aside className="rounded-2xl bg-[#071b33] p-6 text-white"><PackageCheck className="size-7 text-sky-300"/><h2 className="mt-4 text-xl font-bold">Buy with confidence</h2><p className="mt-2 text-sm leading-6 text-slate-300">Stock and pricing are checked again when your order is placed. You receive an order reference and can follow fulfilment from your account.</p></aside></div>
