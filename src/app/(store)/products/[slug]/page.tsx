  ...
  return <main className="bg-white pb-14"><div className="mx-auto max-w-7xl px-4 pt-5 sm:px-6 lg:px-8">
    ...
    <div className="mt-6 grid items-start gap-8 lg:grid-cols-[minmax(0,1.08fr)_minmax(380px,.92fr)] lg:gap-12">
      ...
      <section className="lg:sticky lg:top-28">
        ...
        <div className="mt-5 grid grid-cols-2 gap-4 text-sm"><Benefit icon={Truck} title="Delivery coverage"><DeliveryBenefit/></Benefit><Benefit icon={ShieldCheck} title="Warranty">{product.warranty ?? "Supplier backed"}</Benefit><Benefit icon={CreditCard} title="Secure payment">Pay securely with Ozow</Benefit><Benefit icon={RotateCcw} title="Returns support"><Link className="text-sky-700 underline" href="/returns-policy">View policy</Link></Benefit></div>
        <ProductShare title={product.name} path={`/products/${product.slug}`}/>
      </section>
    </div>
    <section className="mt-14 border-t border-slate-200 pt-10"><h2 className="text-2xl font-bold text-slate-950">About this product</h2><div className="mt-4 max-w-4xl whitespace-pre-line leading-8 text-slate-700">{product.description ?? product.shortDescription ?? "Product information is currently unavailable."}</div></section>
