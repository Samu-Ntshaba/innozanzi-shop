...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main>
    ...
    <article>
      ...
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:py-14"><BlogContent content={post.content}/>
        {sources.length ? <aside className="mt-12 border-t border-slate-200 pt-8"><h2 className="text-xl font-bold text-slate-950">Sources and further reading</h2><p className="mt-2 text-sm leading-6 text-slate-600">This article was prepared from the sources below and reviewed by our team before publication.</p><ol className="mt-5 space-y-3 text-sm">{sources.map((source, index) => <li key={`${source.url}-${index}`}><a href={source.url} target="_blank" rel="noreferrer" className="font-medium text-sky-700 underline decoration-sky-300 underline-offset-4">{source.title || source.url}</a></li>)}</ol></aside> : null}
      </div>
    </article>
  </main>;
}
