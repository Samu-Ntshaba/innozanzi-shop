...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  const post = await getPost((await params).slug);
  if (!post) notFound();
  const relatedProducts = await resolveCatalogueReferences(blogSourceEntries(post.sources).products.flatMap(product => product.catalogueReference ? [product.catalogueReference] : []));
  ...
  return <main>
    ...
    <article>
      ...
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:py-14"><BlogContent content={post.content}/>{relatedProducts.length ? <section className="mt-12 border-t pt-8"><h2 className="text-2xl font-bold text-slate-950">Products related to this guide</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{relatedProducts.map(product => <Link key={product.reference} href={product.publicPath} className="rounded-xl border bg-white p-4 transition hover:border-sky-400"><span className="text-xs font-bold uppercase tracking-wide text-sky-700">{product.brand ?? "Technology"}</span><span className="mt-1 block font-semibold text-slate-950">{product.name}</span><span className="mt-2 block text-sm text-sky-700">View current product →</span></Link>)}</div></section> : null}</div>
    </article>
  </main>;
}
