...
export default async function ActivateAccountPage({ searchParams }: { searchParams: Promise<{ error?: string;token?:string;email?:string;partner?:string }> }) {
  const { error,token,email,partner } = await searchParams;
  ...
  if(token&&email){
    ...
    if(!invitation.valid)return <AuthShell eyebrow="Secure partner activation" title="Activation link unavailable" description="This activation link is invalid, expired, or has already been used. Ask Innozanzi to resend the invitation." footer={<>Need assistance? <Link className="font-bold text-sky-700 hover:underline" href="/contact">Contact Innozanzi support</Link></>}><Link className="mt-6 inline-block rounded-lg bg-[#0a6ed1] px-5 py-3 font-bold text-white" href="/sign-in">Return to sign in</Link></AuthShell>;
  ...
  if(partner==="1"&&error==="invalid")return <AuthShell eyebrow="Secure partner activation" title="Activation link unavailable" description={errorMessage} footer={<>Need assistance? <Link className="font-bold text-sky-700 hover:underline" href="/contact">Contact Innozanzi support</Link></>}><Link className="mt-6 inline-block rounded-lg bg-[#0a6ed1] px-5 py-3 font-bold text-white" href="/sign-in">Return to sign in</Link></AuthShell>;
