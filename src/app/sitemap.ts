...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
  ...
  const[{siteUrl:base},products,supplierProducts,categories,pages,posts,excluded]=await Promise.all([
    globalSeoSettings(),
    prisma.product.findMany({where:{status:"PUBLISHED",deletedAt:null,isTestData:false},select:{id:true,slug:true,updatedAt:true}}),
    prisma.supplierCatalogueProduct.findMany({where:{active:true,images:{isEmpty:false}},select:{id:true,slug:true,updatedAt:true}}),
  ...
  return[
    ...
    ...products.filter(x=>!blocked.has(`PRODUCT:${x.id}`)).map(x=>({url:`${base}/products/${x.slug}`,lastModified:x.updatedAt,changeFrequency:"weekly" as const,priority:.8})),
    ...supplierProducts.filter(x=>!blocked.has(`SUPPLIER_PRODUCT:${x.id}`)).map(x=>({url:`${base}/supplier-products/${x.slug}`,lastModified:x.updatedAt,changeFrequency:"weekly" as const,priority:.7})),
