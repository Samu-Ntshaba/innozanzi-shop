...
export default async function SocialMarketingPage({ searchParams }: { searchParams: Promise<{ result?: string; stage?: string; type?: string; date?: string; search?: string }> }) {
  ...
  return <AdminPage title="Social media" description="One useful, ready-to-post social image and caption is prepared each day for South African customers." actions={<Link className="text-sm font-semibold text-sky-700" href="/admin/marketing/settings">Settings</Link>}>
    ...
    <Panel title="One strong post each day" description="The system rotates through in-stock products and different customer angles. Artwork is a clean 1080 × 1350 product image with no generated text, fake interface, border or promotional claim.">
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><span className="text-xs font-black uppercase tracking-wide text-sky-700">Daily post</span><h3 className="mt-1 font-bold">Useful product discovery</h3><p className="mt-1 text-sm text-slate-600">Real catalogue image · natural caption · product link · focused local hashtags</p></div>
