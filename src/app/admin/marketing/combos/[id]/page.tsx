...
export default async function ComboDetail({params}:{params:Promise<{id:string}>}){
  ...
  const campaign=await prisma.comboCampaign.findUnique({where:{id},include:{items:{include:{product:{include:{inventory:true}},supplierCatalogueProduct:true}},events:{orderBy:{createdAt:"desc"},take:50},quotationSnapshots:true}});
  ...
  return <AdminPage title={campaign.name} description={campaign.headline} actions={<><Link className={secondaryButtonClass} href="/admin/marketing/combos">All combos</Link><Link className={secondaryButtonClass} href={`/combos/${campaign.slug}`} target="_blank">Public preview</Link></>}>
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_360px]"><Panel title="Included products"><table className={tableClass}><thead><tr><th>Product</th><th>Qty</th><th>Available</th><th>Normal</th><th>Internal cost</th></tr></thead><tbody>{campaign.items.map(x=><tr key={x.id}><td>{x.productName}<small className="block">{x.sku}</small></td><td>{x.quantity}</td><td>{x.supplierCatalogueProduct?.stock??x.product?.inventory.reduce((n,i)=>n+Math.max(0,i.onHand-i.reserved),0)??0}</td><td>R {Number(x.unitNormalPrice).toFixed(2)}</td><td className="text-red-800">R {Number(x.unitCost).toFixed(2)}<small className="block font-bold">PRIVATE</small></td></tr>)}</tbody></table></Panel><div className="space-y-5">
