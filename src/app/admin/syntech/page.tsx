...
export default async function Page(){
  ...
  const [feed,runs,count,clock]=await Promise.all([
    prisma.supplierFeed.findFirst({where:{provider:"SYNTECH"},include:{supplier:true}}),
    prisma.supplierSyncRun.findMany({where:{feed:{provider:"SYNTECH"}},orderBy:{startedAt:"desc"},take:20}),
    prisma.supplierCatalogueProduct.count({where:{active:true,feed:{provider:"SYNTECH"}}}),
    prisma.$queryRaw<Array<{now:Date}>>`SELECT NOW() AS now`,
  ]);
  const configured=Boolean(process.env.SYNTECH_FULL_FEED_URL&&process.env.SYNTECH_UPDATE_FEED_URL);
  const latest=runs[0];
  const fresh=Boolean(feed?.lastSuccessAt&&clock[0].now.getTime()-feed.lastSuccessAt.getTime()<=MAX_FEED_AGE_MS);
  const healthy=configured&&fresh&&latest?.status==="SUCCEEDED"&&!feed?.lastError;
  const healthMessage=!configured
    ?"Feed credentials are not configured."
    :!fresh
      ?`Supplier data is stale. Last successful refresh: ${feed?.lastSuccessAt?.toLocaleString("en-ZA")??"never"}. Check or redeploy the Railway supplier cron before relying on stock or promotions.`
      :feed?.lastError??"A successful feed run has not been recorded.";

  return <AdminPage title="Supplier feed management" description="Monitor the daily Syntech catalogue, stock and price refresh.">
    <div className="grid gap-5 lg:grid-cols-3">
      ...
      <Panel title="Daily synchronisation"><p className="text-sm text-slate-600">Last success: {feed?.lastSuccessAt?.toLocaleString("en-ZA")??"Never"}<br/>Next expected refresh: {feed?.nextSyncAt?.toLocaleString("en-ZA")??"After first sync"}</p><div className="mt-4 flex flex-wrap gap-2"><form action={runSyntechIncrementalSync}><button disabled={!configured} className={buttonClass}>Refresh prices now</button></form><form action={runSyntechFullSync}><button disabled={!configured} className={secondaryButtonClass}>Rebuild catalogue now</button></form></div></Panel>
