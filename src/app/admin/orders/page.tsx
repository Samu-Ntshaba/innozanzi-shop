...
export default async function Page({searchParams}:{searchParams:Promise<{supplier?:string;status?:string}>}) {
  ...
  const [rows,suppliers]=await Promise.all([getAdminOrders(selectedSupplier,selectedStatus),prisma.supplier.findMany({where:{deletedAt:null,isActive:true},select:{id:true,companyName:true},orderBy:{companyName:"asc"}})]);
  ...
  return <AdminPage title="Order fulfilment" description="Orders appear only after payment verification. Open an order to publish controlled fulfilment updates.">
    <Panel><form className="flex flex-wrap gap-2"><select className={`${inputClass} min-w-64`} name="supplier" defaultValue={selectedSupplier}><option value="">All distributors and local stock</option>{suppliers.map(supplier=><option key={supplier.id} value={supplier.id}>{supplier.companyName}</option>)}</select><select className={`${inputClass} min-w-52`} name="status" defaultValue={selectedStatus}><option value="">All order stages</option>{statuses.map(status=><option key={status} value={status}>{status.replaceAll("_"," ")}</option>)}</select><button className={secondaryButtonClass}>Filter orders</button>{selectedSupplier||selectedStatus?<Link className={secondaryButtonClass} href="/admin/orders">Clear</Link>:null}</form></Panel>
