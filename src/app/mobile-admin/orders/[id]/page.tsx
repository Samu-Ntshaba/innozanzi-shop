...
export default async function MobileOrderDetail({params}:{params:Promise<{id:string}>}){
  ...
  return <><div className="flex items-center justify-between gap-3"><div><Link href="/mobile-admin/orders" className="text-sm font-bold text-sky-700">← Orders</Link><h1 className="mt-2 text-2xl font-black text-[#071b33]">{order.orderNumber}</h1></div><span className="rounded-full bg-sky-100 px-3 py-2 text-xs font-black text-sky-800">{order.status.replaceAll("_"," ")}</span></div>
    {canOperateOrder?<div className="mt-5 rounded-2xl bg-[#071b33] p-5 text-white"><p className="text-xs font-bold uppercase tracking-wider text-sky-300">Next normal action</p><p className="mt-2 text-xl font-black">{operation.label}</p>{operation.blocker?<p className="mt-2 text-sm text-amber-200">{operation.blocker}</p>:null}{operation.anchor&&!operation.targetStatus?<a className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-sky-600 px-4 font-bold" href={operation.kind==="BLOCKED"?"#distributor-assignment":`#${operation.anchor}`}>Continue in this order</a>:null}</div>:<div className="mt-5 rounded-2xl border border-amber-300 bg-amber-50 p-5 text-amber-950"><p className="text-xl font-black">Payment confirmation pending</p><p className="mt-2 text-sm">Fulfilment controls remain locked until verified gateway evidence is recorded.</p><Link className="mt-3 inline-block font-bold text-sky-700 underline" href="/admin/payments">Review payment evidence</Link></div>}
    {canOperateOrder&&operation.targetStatus?<Section title="Next action"><form id="normal-progression" action={setOrderStatus} className={`${card} grid gap-3 p-4`}><input type="hidden" name="id" value={order.id}/><input type="hidden" name="status" value={operation.targetStatus}/><textarea className={`${inputClass} min-h-16`} name="internalNote" placeholder="Optional internal note"/><button className="min-h-12 rounded-xl bg-sky-700 px-4 font-bold text-white">{operation.label}</button></form></Section>:null}
    ...
    {canOperateOrder?<><OrderDistributorAssignment orderId={order.id} items={order.items.filter(item=>!item.supplierId).map(item=>({id:item.id,productName:item.productName,sku:item.sku,supplierSku:item.supplierSku,costPrice:item.costPrice?.toString()??null}))} distributors={eligibleDistributors}/><div id="supplier-order"><Section title="Supplier order"><OrderSupplierSetup orderId={order.id} suppliers={missingSuppliers}/></Section></div><div id="supplier-delivery"><Section title="Supplier & tracking"><OrderSupplierShipments orderId={order.id} groups={order.procurements}/></Section></div></>:null}
    ...
    {canOperateOrder&&allowedOrderTransitions(order.status).includes("CANCELLED")?<Section title="Exception actions"><p className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">Cancellation and refunds remain finance-controlled exception actions. Open the full order workspace when this exception is required.</p></Section>:null}
  </>;
}
