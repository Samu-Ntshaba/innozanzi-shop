...
WITH ranked AS (
 ...
 ORDER BY (p.active AND p.availability='IN_STOCK' AND p.stock>0 AND p."costPrice">0 AND CARDINALITY(p.images)>0 AND s."purchasingEnabled" AND f.enabled AND p."lastSeenAt">=${since} AND f."lastSuccessAt">=${since}) DESC,
