...
async function syncAllSuppliersUnlocked(){
 ...
 for(const provider of ["SYNTECH","PINNACLE"]){
  ...
  try{results.push({provider,status:"SUCCEEDED",...await syncSyntechFeed("FULL")});}catch{results.push({provider,status:"FAILED"});}
 }
 return results;
}
...
export async function syncAllSuppliers(){
 ...
 try{return await syncAllSuppliersUnlocked();}finally{await prisma.siteSetting.deleteMany({where:{key:"supplier.sync.lease",value:{path:["owner"],equals:owner}}});}
}
