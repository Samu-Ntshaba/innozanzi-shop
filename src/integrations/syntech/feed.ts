...
import { supplierIdentity } from "@/integrations/suppliers/identity";
...
export async function ensureSyntechFeed(){
  ...
  const supplier=await prisma.supplier.upsert({where:{id:supplierId},update:{},create:{approvalStatus:"APPROVED",purchasingEnabled:true,id:supplierId,companyName:"Syntech Distribution",website:"https://www.syntech.co.za",isActive:true}});
  return prisma.supplierFeed.upsert({where:{supplierId_provider:{supplierId:supplier.id,provider:"SYNTECH"}},update:{scheduleMinutes:1440},create:{supplierId:supplier.id,provider:"SYNTECH",adapter:"SYNTECH_JSON",fullFeedUrl:"ENV:SYNTECH_FULL_FEED_URL",updateFeedUrl:"ENV:SYNTECH_UPDATE_FEED_URL",scheduleMinutes:1440,nextSyncAt:new Date()},include:{supplier:true}})
}
export async function syncSyntechFeed(mode:SyncMode,actorId?:string){
  const feed=await ensureSyntechFeed();if(!feed.enabled)throw new Error("Supplier feed is paused");const run=await prisma.supplierSyncRun.create({data:{feedId:feed.id,supplierId:feed.supplierId,mode,status:"RUNNING",triggeredById:actorId}});const now=new Date();
  try{const parsed=parseSyntechFeed(await new SyntechJsonAdapter().fetch(mode));if(parsed.syntechstock.currency.trim().toUpperCase()!=="ZAR")throw new Error("Unexpected supplier currency");if(new Set(parsed.syntechstock.products.map(p=>p.sku)).size!==parsed.syntechstock.products.length)throw new Error("Duplicate supplier SKU in feed");let added=0,updated=0,skipped=0;
    if(mode==="FULL"){
      ...
      await prisma.$transaction(rows.map(data=>prisma.supplierCatalogueProduct.upsert({where:{feedId_supplierProductId:{feedId:feed.id,supplierProductId:data.supplierProductId}},update:data,create:data})),{timeout:120_000});
    ...
    await prisma.supplierSyncRun.update({where:{id:run.id},data:{status:"SUCCEEDED",finishedAt:new Date(),recordsReceived:parsed.syntechstock.products.length,recordsAdded:added,recordsUpdated:updated,recordsRemoved:removed,recordsSkipped:skipped,diagnostics:{currency:parsed.syntechstock.currency,declaredCount:parsed.syntechstock.count}}});await prisma.supplierFeed.update({where:{id:feed.id},data:{lastSuccessAt:new Date(),lastError:null,nextSyncAt:next,lastIncrementalSyncAt:new Date()}});await reconcileOfferDisplay();return{runId:run.id,total:parsed.syntechstock.products.length,added,updated,removed,skipped};
