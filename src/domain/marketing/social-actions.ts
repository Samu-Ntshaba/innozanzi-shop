...
export async function saveSocialSettings(formData: FormData) {
  ...
  const data = z.object({ recipientEmail: z.string().trim().email(), generationHour: z.coerce.number().int().min(0).max(23), brandDirection: z.string().trim().min(20).max(1_000), completionWindowDays: z.coerce.number().int().min(1).max(90), enabled: z.string().optional() }).parse(Object.fromEntries(formData));
  const values = [["social.recipientEmail", data.recipientEmail, "Recipient of daily, ready-to-post social content."], ["social.generationHour", data.generationHour, "Preferred generation hour in Africa/Johannesburg."], ["social.brandDirection", data.brandDirection, "Visual direction applied to generated feature artwork."], ["social.enabled", data.enabled === "on", "Whether the daily social content cron is enabled."], ["orders.completionWindowDays", data.completionWindowDays, "Days after delivery before an order closes automatically when no return is open."]] as const;
