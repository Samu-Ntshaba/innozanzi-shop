...
export async function generateComboDraft(formData:FormData){
  ...
  const products=await prisma.product.findMany({where:{status:"PUBLISHED",deletedAt:null,isTestData:false},include:{inventory:true,suppliers:{where:{isPreferred:true},take:1}},take:80});
  ...
  if(candidates.length<2)throw new Error("At least two stocked products with verified costs are required.");
