...
const STAGE_COPY: Record<string, { phase: string; reason: string; next: string; owner: string; customer: string }> = {
  ...
  IN_TRANSIT: { phase: "In transit", reason: "The parcel is travelling to the customer.", next: "Confirm delivery", owner: "Order operations", customer: "On the way" },
  OUT_FOR_DELIVERY: { phase: "Out for delivery", reason: "The delivery provider has the parcel on its final delivery route.", next: "Confirm delivery", owner: "Order operations", customer: "Out for delivery" },
