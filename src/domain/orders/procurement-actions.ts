...
export async function saveDistributorShipment(formData:FormData){
  const context=await requirePermission("orders.update");
  const data=z.object({orderId:z.string().uuid(),procurementId:z.string().uuid(),status:z.enum(["PENDING","SHIPPED","IN_TRANSIT","OUT_FOR_DELIVERY","DELIVERED"]),deliveryCompany:z.string().trim().min(2).max(160),trackingNumber:z.string().trim().max(120).optional(),trackingUrl:z.string().url().optional(),estimatedDeliveryAt:z.coerce.date().optional(),deliveryWindow:z.string().max(120).optional(),deliveryInstructions:z.string().max(2000).optional()}).parse({orderId:formData.get("orderId"),procurementId:formData.get("procurementId"),status:formData.get("status"),deliveryCompany:formData.get("deliveryCompany"),trackingNumber:optional(formData.get("trackingNumber")),trackingUrl:optional(formData.get("trackingUrl")),estimatedDeliveryAt:optional(formData.get("estimatedDeliveryAt")),deliveryWindow:optional(formData.get("deliveryWindow")),deliveryInstructions:optional(formData.get("deliveryInstructions"))});
  let message:ReturnType<typeof emailTemplates.orderStatus>|null=null;
  const result=await prisma.$transaction(async tx=>{
    await tx.$queryRaw`SELECT id FROM "Order" WHERE id = ${data.orderId}::uuid FOR UPDATE`;
    const order=await tx.order.findUniqueOrThrow({where:{id:data.orderId},include:{procurements:{include:{shipments:true}}}});
    ...
    const shipment=existing?await tx.shipment.update({where:{id:existing.id},data:{status:data.status,deliveryCompany:data.deliveryCompany,carrier:data.deliveryCompany,trackingNumber:data.trackingNumber??null,trackingUrl:data.trackingUrl??null,estimatedDeliveryAt:data.estimatedDeliveryAt??null,deliveryInstructions:data.deliveryInstructions??null,shippedAt:data.status==="SHIPPED"&& !existing.shippedAt?new Date():undefined,deliveredAt:data.status==="DELIVERED"?new Date():undefined}}):await tx.shipment.create({data:{orderId:order.id,procurementId:procurement.id,status:data.status,deliveryCompany:data.deliveryCompany,carrier:data.deliveryCompany,trackingNumber:data.trackingNumber??null,trackingUrl:data.trackingUrl??null,estimatedDeliveryAt:data.estimatedDeliveryAt??null,deliveryInstructions:data.deliveryInstructions??null,deliveryNoteNumber:`DN-${order.orderNumber}-${procurement.id.slice(0,6).toUpperCase()}`,shippedAt:data.status==="SHIPPED"?new Date():undefined,deliveredAt:data.status==="DELIVERED"?new Date():undefined}});
    ...
    const publicNote=data.status==="SHIPPED"?`Your order has shipped with ${data.deliveryCompany}.${data.trackingNumber?` Tracking: ${data.trackingNumber}.`:""}`:data.status==="OUT_FOR_DELIVERY"?"Your order is out for delivery.":data.status==="DELIVERED"?"Your order has been delivered.":data.estimatedDeliveryAt?`Expected delivery: ${data.estimatedDeliveryAt.toLocaleDateString("en-ZA")}${data.deliveryWindow?` · ${data.deliveryWindow}`:""}.`:"Delivery information was updated.";
    ...
    await tx.auditLog.create({data:{actorId:context.user.id,action:"order.distributor-shipment.update",entityType:"Shipment",entityId:shipment.id,after:{supplierId:procurement.supplierId,status:data.status,courier:data.deliveryCompany,trackingNumber:data.trackingNumber,source:"DISTRIBUTOR"}}});
