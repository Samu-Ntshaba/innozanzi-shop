...
The checkout shows the product price, VAT and delivery charges that apply to the order. Supplier prices and stock can change, and the website is refreshed regularly, but an obvious pricing error does not require us to supply at the incorrect price. Ozow and PayFast process online payments when enabled. Manual EFT and uploaded payment proofs are not offered for new purchases. Payment is confirmed through server-side provider verification. We do not separately charge output VAT while Innozanzi is not VAT registered. Coupon and promotion eligibility, expiry, limits and final savings are confirmed at checkout; offers do not stack and are unavailable where their application would breach the protected selling-price floor.
