...
Your control
Optional analytics and advertising cookies are off until you choose them. The Google Ads base tag loads with consent denied by default and can send cookieless consent and measurement signals to Google before you choose; these may include network and device information. Cookie preferences are available on this Cookie Policy page to change your choice. Choosing Essential only disables optional cookies and consent-based analytics collection and browsing-history personalisation and clears accessible analytics and advertising cookies and our recommendation cookie. Browser settings can remove remaining cookies, including ones set on other domains. Existing provider records are not automatically erased by withdrawing consent; contact support@innozanzi.co.za for privacy requests.
...
The checkout shows the product price, VAT and delivery charges that apply to the order. Supplier prices and stock can change, and the website is refreshed regularly, but an obvious pricing error does not require us to supply at the incorrect price. Ozow processes instant bank payments when enabled. If manual EFT is enabled, checkout displays the approved company banking details and requires proof of payment; an uploaded document is evidence only and fulfilment begins only after finance verifies cleared funds. Payment is confirmed through server-side provider verification or controlled finance review. We do not separately charge output VAT while Innozanzi is not VAT registered. Coupon and promotion eligibility, expiry, limits and final savings are confirmed at checkout; offers do not stack and are unavailable where their application would breach the protected selling-price floor.
...
Google Ads advertising cookies and full measurement require their own optional choice on /policies/cookies. The base tag loads with consent denied and can send cookieless consent and measurement signals before permission, including network and device information. We keep personalised advertising disabled and do not send names, email addresses or delivery addresses through this tag. Google may process service and measurement information internationally as explained in its privacy information.
