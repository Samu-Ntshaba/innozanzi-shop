...
async function candidates(intent: ShoppingIntent, target: ShoppingTarget): Promise<AIProduct[]> {
    ...
    const rows = await prisma.supplierCatalogueProduct.findMany({ where: {
            ...{ active: true, availability: "IN_STOCK", stock: { gt: 0 }, costPrice: { gt: 0 }, images: { isEmpty: false }, AND: [strictCatalogueScope(target), ...(intent.brand ? [{ brand: { equals: intent.brand, mode: "insensitive" as const } }] : []), ...(focused.includes("gaming") ? [{ OR: [{ name: { contains: "gaming", mode: "insensitive" as const } }, { categoryPath: { contains: "gaming", mode: "insensitive" as const } }] }] : []), ...(generalSearch ? [generalSearch] : [])] },
            ...await sellableSupplierWhere()
        }, orderBy: [{ promotionalPrice: "asc" }, { sourceUpdatedAt: "desc" }], take: 80 });
