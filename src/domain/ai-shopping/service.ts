...
import { productHasCapability, supplierCapabilityWhere } from "@/domain/catalogue/taxonomy";
...
async function pcBuild(intent: ShoppingIntent) {
    ...
    for (const key of required) {
        ...
        if (!step)
            continue;
        const options = step.products.filter(item => pcPartCompatibility(key, item, selections as CompatibilitySelection).kind !== "bad").sort((a, b) => Number(a.price) - Number(b.price));
        if (options.length)
            selections[key] = options[Math.min(options.length - 1, key === "graphics" ? Math.floor(options.length * .35) : Math.floor(options.length * .2))];
    }
    let chosen = Object.entries(selections);
    let total = chosen.reduce((sum, [, item]) => sum + Number(item.price), 0);
    if (total > budget) {
        for (const [key] of [...chosen].sort((a, b) => Number(b[1].price) - Number(a[1].price))) {
            const step = steps.find(item => item.key === key);
            const replacement = step?.products.filter(item => Number(item.price) < Number(selections[key].price) && pcPartCompatibility(key, item, Object.fromEntries(Object.entries(selections).filter(([other]) => other !== key)) as CompatibilitySelection).kind !== "bad").sort((a, b) => Number(a.price) - Number(b.price))[0];
            if (replacement) {
                selections[key] = replacement;
                total = Object.values(selections).reduce((sum, item) => sum + Number(item.price), 0);
            }
            if (total <= budget)
                break;
        }
        chosen = Object.entries(selections);
    }
    if (chosen.length < 6 || (intent.maxBudget > 0 && total > budget))
        throw new Error("NO_BUILD");
    const products = chosen.map(([key, item]) => ({ ...item, price: Number(item.price), category: steps.find(step => step.key === key)?.shortTitle ?? key, reason: key }));
    const response = await getOpenAIClient().responses.create({ model: model(), store: false, max_output_tokens: 120, instructions: "Write one short, plain-English sentence explaining why this available PC build fits the stated use and budget. Do not invent specifications or promise performance.", input: JSON.stringify({ intent, total, parts: products.map(item => ({ name: item.name, category: item.category, price: item.price, specifications: item.specifications })) }) }, { timeout: 25000 });
...
export async function recommend(input: string, source?: string): Promise<AIShoppingResult> {
    ...
    try {
        const parsed = await understand(input), target = shoppingTarget(input), explicitBuild = isExplicitBuildRequest(input), budget = explicitBudget(input);
        intent = { ...parsed.intent, kind: explicitBuild ? "PC_BUILD" : "PRODUCT", maxBudget: budget ?? parsed.intent.maxBudget };
        usage = parsed.usage ?? usage;
        const result = intent.kind === "PC_BUILD" ? await pcBuild(intent) : await productRecommendation(intent, await candidates(intent, target));
        usage = { input_tokens: usage.input_tokens + (result.usage?.input_tokens ?? 0), output_tokens: usage.output_tokens + (result.usage?.output_tokens ?? 0), total_tokens: usage.total_tokens + (result.usage?.total_tokens ?? 0) };
        const total = "total" in result && typeof result.total === "number" ? result.total : Number(result.products[0]?.price ?? 0);
        ...
        await prisma.aIUsage.create({ data: { userId: identity.userId, anonymousSessionId: identity.anonymousSessionId, recommendationId, model: model(), intentType: intent?.kind, inputTokens: usage.input_tokens, outputTokens: usage.output_tokens, totalTokens: usage.total_tokens, estimatedCost: estimateAIRequestCost(usage.input_tokens, usage.output_tokens), requestStatus: "FAILED", responseTimeMs: Date.now() - started, errorCode: error instanceof Error ? error.message.slice(0, 80) : "UNKNOWN", metadata: intent ? { source, category: intent.category, brand: intent.brand, useCase: intent.useCase, budget: intent.maxBudget, target: shoppingTarget(input) } : { source } } }).catch(() => undefined);
        if (error instanceof Error && ["NO_MATCHES", "NO_BUILD"].includes(error.message)) {
            const target = intent?.kind === "PC_BUILD" ? "compatible PC build" : targetLabel(shoppingTarget(input)), budget = intent?.maxBudget;
            throw new Error(`NO_MATCHES|Unfortunately, we couldn't find an in-stock ${target}${budget ? ` within R${budget.toLocaleString("en-ZA")}` : " matching that request"}. Our team can check stock or help source it. Send a product request below, or browse the catalogue.`);
        }
        throw error;
    }
}
