...
export async function getHomepageCatalogue() {
    try {
        ...
        const [supplierCategories, featured, specials, popular, brands, supplierNewest, total, inStock, laptopsAndComputers, monitors, accessories, networking, powerAndBackup, promotions, unboxed, lastChance] = await Promise.all([
            ...
            prisma.supplierCatalogueProduct.findMany({ where: {
                    ...{ ...merchandiseWhere, costPrice: { gt: 0 }, OR: [{ categoryPath: { startsWith: "Computers/Notebooks/" } }, { categoryPath: { startsWith: "Computers/Desktop computers/" } }, { categoryPath: { startsWith: "Computers/AIO computers" } }, { categoryPath: { startsWith: "Computers/Mini PCs/Complete systems" } }] },
                    ...await sellableSupplierWhere()
                }, orderBy: { costPrice: "asc" }, take: 12, select: supplierCardSelect }),
            prisma.supplierCatalogueProduct.findMany({ where: {
                    ...{ ...merchandiseWhere, costPrice: { gt: 0 }, categoryPath: { startsWith: "Computer peripherals/Monitors/" } },
                    ...await sellableSupplierWhere()
                }, orderBy: { costPrice: "asc" }, take: 12, select: supplierCardSelect }),
            prisma.supplierCatalogueProduct.findMany({ where: {
                    ...{ ...merchandiseWhere, costPrice: { gt: 0 }, OR: [{ categoryPath: { startsWith: "Computer peripherals/Keyboards/" } }, { categoryPath: { startsWith: "Computer peripherals/Mice/" } }, { categoryPath: { startsWith: "Computer peripherals/Headsets/" } }, { categoryPath: { startsWith: "Computer peripherals/Hubs & docking stations" } }, { categoryPath: { startsWith: "Computer peripherals/Stands and cooling" } }] },
                    ...await sellableSupplierWhere()
                }, orderBy: { costPrice: "asc" }, take: 12, select: supplierCardSelect }),
            prisma.supplierCatalogueProduct.findMany({ where: {
            ...
            prisma.supplierCatalogueProduct.findMany({ where: {
                    ...{ ...merchandiseWhere, stock: { gt: 0 }, costPrice: { gt: 0 }, promotionalPrice: { not: null }, AND: [{ OR: [{ promotionStartsAt: null }, { promotionStartsAt: { lte: now } }] }, { OR: [{ promotionEndsAt: null }, { promotionEndsAt: { gte: now } }] }] },
                    ...await sellableSupplierWhere()
...
export async function getCatalogue(input: {
    ...
    try {
        ...
        const [supplierProducts, manualProducts] = await Promise.all([
            supplierTake ? prisma.supplierCatalogueProduct.findMany({ where: {
                    ...supplierWhere,
                    ...await sellableSupplierWhere()
                }, orderBy: input.sort === "name" ? { name: "asc" } : input.sort === "stock" ? { stock: "desc" } : input.sort === "oldest" ? { sourceUpdatedAt: "asc" } : { sourceUpdatedAt: "desc" }, skip, take: supplierTake, select: supplierCardSelect }) : [],
