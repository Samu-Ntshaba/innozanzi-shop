...
export async function resolveQuotationCart(cart: CurrentCart, markup: Decimal): Promise<QuotationSourceLine[]> {
    ...
    const local = await Promise.all(cart.items.map(async (item): Promise<QuotationSourceLine> => {
        ...
        if (item.quantity > available)
            throw new Error(`${item.product.name} only has ${available} units available.`);
    ...
    if (products.length !== cart.supplierItems.length)
        throw new Error("A supplier product is no longer available. Review your quotation list.");
    const supplier = await Promise.all(cart.supplierItems.map(async (item): Promise<QuotationSourceLine> => {
        const product = products.find((row) => row.supplierId === item.supplierId && row.supplierProductId === item.supplierProductId);
        if (!product)
            throw new Error(`Supplier item ${item.supplierSku} is no longer available.`);
        if(!product.feed.enabled||!product.feed.lastSuccessAt||Date.now()-product.feed.lastSuccessAt.getTime()>settings.freshnessHours*3600000||!product.supplier.purchasingEnabled||product.supplier.approvalStatus!=="APPROVED"||Date.now()-product.lastSeenAt.getTime()>settings.freshnessHours*3600000)throw new Error(`${product.name} is awaiting a fresh supplier price. Please contact us.`);
