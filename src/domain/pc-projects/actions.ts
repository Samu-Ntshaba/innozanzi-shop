...
export async function continuePcProjectAuthentication(formData: FormData) {
    ...
    const params = new URLSearchParams({ draftName: data.name, draftType: data.buildType, draftUse: data.intendedUse, draftTarget: data.performanceTarget });
    data.gamePreferences.forEach(game => params.append("draftGame", game));
...
export async function savePcProjectPart(input: {
    projectId: string;
    stepKey: string;
    productId: string;
}) {
    ...
    if (!project)
        throw new Error("PC project not found.");
    const existing = project.items.find(item => item.stepKey === data.stepKey);
    if (existing?.purchasedAt)
        throw new Error("Purchased components cannot be replaced.");
    if (project.buildType === "PC_ONLY" && ["monitor", "keyboard", "mouse", "audio"].includes(data.stepKey))
        throw new Error("Desk peripherals are not part of this PC-only project.");
    const product = await prisma.supplierCatalogueProduct.findFirst({ where: {
            ...{ id: data.productId, active: true, availability: "IN_STOCK", stock: { gt: 0 } },
            ...await sellableSupplierWhere()
        }, select: { id: true, name: true, supplierSku: true, categoryPath: true, images: true, specifications: true, costPrice: true, recommendedRetail: true, promotionalPrice: true, promotionStartsAt: true, promotionEndsAt: true } });
    if (!product?.costPrice)
        throw new Error("This component is no longer available.");
    if (!productMatchesPcBuildStep(data.stepKey as PcBuildStepKey, product.categoryPath))
        throw new Error("That product does not belong in this PC component stage.");
    ...
    const compatibility = pcPartCompatibility(data.stepKey, { name: product.name, categoryPath: product.categoryPath, specifications: product.specifications }, selected);
    if (compatibility.kind === "bad")
        throw new Error(compatibility.label);
    const retail = await supplierRetailPrice({ costPrice: product.costPrice, recommendedRetail: product.recommendedRetail, promotionalPrice: product.promotionalPrice, promotionStartsAt: product.promotionStartsAt, promotionEndsAt: product.promotionEndsAt, special: isDailySpecial(product.id), productKey:`SUPPLIER:${product.id}` }), price = retail.salePrice ?? retail.regularPrice;
    ...
    revalidatePath(`/build-a-pc`);
    revalidatePath(`/account/pc-projects/${project.id}`);
    return { saved: true };
}
export async function addPcProjectPartsToCart(formData: FormData) {
    ...
    const project = await prisma.pcProject.findFirst({ where: { id: projectId, userId: ctx.user.id }, include: { items: true } });
    if (!project)
        throw new Error("PC project not found.");
    const items = project.items.filter(item => ids.includes(item.id) && !item.purchasedAt);
    if (items.length !== new Set(ids).size)
        throw new Error("One or more selected parts cannot be purchased.");
    const live = await prisma.supplierCatalogueProduct.findMany({ where: {
            ...{ id: { in: items.map(item => item.supplierProductId) }, active: true, availability: "IN_STOCK", stock: { gt: 0 }, costPrice: { gt: 0 } },
            ...await sellableSupplierWhere()
        } });
    if (live.length !== items.length)
        redirect(`/account/pc-projects/${project.id}?error=availability`);
    const prices = await Promise.all(await Promise.all(live.map(async (item) => { const p = await supplierRetailPrice({ costPrice: item.costPrice!, recommendedRetail: item.recommendedRetail, promotionalPrice: item.promotionalPrice, promotionStartsAt: item.promotionStartsAt, promotionEndsAt: item.promotionEndsAt }); return Number(p.salePrice ?? p.regularPrice); })));
    const total = prices.reduce((sum, price) => sum + price, 0);
    if (!progressivePurchaseEligibility(total).eligible)
        redirect(`/account/pc-projects/${project.id}?error=minimum`);
    ...
    revalidatePath("/cart");
    redirect("/cart?status=project-parts-added");
}
export async function analysePcProject(formData: FormData) {
    ...
    if (!project)
        throw new Error("PC project not found.");
    if (!requiredKeys.every(key => project.items.some(item => item.stepKey === key)))
        throw new Error("Complete the required configuration first.");
    const hardware = project.items.map(item => ({ type: item.stepKey, name: item.productName, specifications: item.specifications }));
    const intent = { use: project.intendedUse, performanceTarget: project.performanceTarget, games: project.gamePreferences };
    const fingerprint = createHash("sha256").update(JSON.stringify({ hardware, intent })).digest("hex");
    let analysis: {
        personality: string;
        gaming: {
            label: string;
