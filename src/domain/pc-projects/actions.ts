...
export async function analysePcProject(formData:FormData){
  ...
  await prisma.pcProject.update({where:{id:project.id},data:{aiAnalysis:analysis,analysisFingerprint:fingerprint}});revalidatePath(`/account/pc-projects/${project.id}`);redirect(`/account/pc-projects/${project.id}#analysis`);
}
