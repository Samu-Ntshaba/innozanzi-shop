...
export async function processPaymentEvent(provider: "PAYSTACK" | "YOCO" | "OZOW" | "PAYFAST", event: PaymentEvent) {
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (["PAID","REFUNDED","PARTIALLY_REFUNDED"].includes(latest.status)) return { duplicate: true, paymentId: payment.id, order: payment.order, amount: payment.amount.toString() };
    assertPaymentEventMatches(event,payment);
    ...
    if (event.status === "PAID") {
      ...
      await tx.notification.create({data:{id:payment.orderId,type:"PAID_ORDER_COMMUNICATION",channel:"INTERNAL",status:"PENDING",subject:`Confirmation for ${payment.order.orderNumber}`,body:"Customer and operations confirmation pending",data:{orderId:payment.orderId}}});
