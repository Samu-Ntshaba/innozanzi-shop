...
export async function recoverPaymentAfterReturn(paymentId:string):Promise<PaymentReturnOutcome>{
  const payment=await prisma.payment.findUnique({where:{id:paymentId},select:{status:true,provider:true,externalReference:true,createdAt:true,partnerQuoteCaseId:true}});
  ...
  if(payment.partnerQuoteCaseId && !(await partnerSalesSettings()).enabled)return "processing";
