...
Browser return query parameters never mark payment successful. A successful return now performs a bounded server-side provider lookup: verified Ozow evidence can finalize the order through the same locked finalizer, while PayFast remains processing until signed ITN evidence arrives. Notifications require a valid provider signature, merchant reference, matching amount/currency and server verification (PayFast VALID endpoint; Ozow transaction lookup). Notifications are bounded and duplicate fields rejected. Provider event IDs are unique, payment rows are locked and processed idempotently. A verified payment with an inventory issue is recorded as paid and flagged for procurement review, rather than losing the financial event. `Payment.fundsAvailableAt` is separate and is never set by a success notification.

Create a Railway Cron service from `railway.payment-email-cron.json`. Every ten minutes it calls the authenticated payment reconciliation endpoint, retries durable verified-payment and paid-order communication jobs, and retries failed email outbox messages. The web service alone does not schedule this work. Monitor every non-zero cron exit and confirm the latest successful run in Railway before treating hosted payments as operationally reliable.
...
Pinnacle is configured as approved, COD, account INN038, with separate purchasing/feed flags and a shared scheduler entry. Store `PINNACLE_XML_FEED_URL`, `PINNACLE_FEED_USERNAME`, and `PINNACLE_FEED_PASSWORD` as private variables on both the web and supplier cron services. The XML mapping covers the verified Pinnacle product-feed schema and rejects empty, duplicate-SKU, or severely incomplete imports. Enable the feed before running a monitored full import; enable purchasing only after that import succeeds and representative price, stock, category, barcode, and image records have been reviewed.
