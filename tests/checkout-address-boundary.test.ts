...
it.each(["PAYFAST", "OZOW"])("does not create an order or initiate %s payment without a delivery address", async (method) => {
...
it("rejects manual EFT before creating an order", async () => {
  const form = new FormData(); form.set("paymentMethod", "EFT");
  expect(await placeRetailOrder({ error: "" }, form)).toEqual({ error: "Please check your delivery and payment details." });
